...
parseMessage s = case words s of
  ...
  ("E" : sev : t : msg) -> LogMessage (Error (read sev :: Int)) (read t :: Int) (unwords msg)
  _ -> Unknown s
...
{- 
  ...
  empty tree.
  A MessageTree should be sorted by timestamp: that is, the timestamp of a LogMessage in any Node should be greater than all timestamps of
  any LogMessage in the left subtree, and less than all timestamps of any LogMessage in the right child.
...
-- postOrder Leaf = []
-- postOrder (Node l m r) = preOrder l ++ preOrder r ++ [m]
...
isError (LogMessage {}) = False

sevGreaterThan :: Int -> LogMessage -> Bool
sevGreaterThan x (LogMessage (Error i) _ _) = i > x
sevGreaterThan _ (LogMessage {}) = False

logMessageToString :: LogMessage -> String
logMessageToString (LogMessage _ _ msg) = msg

whatWentWrong :: [LogMessage] -> [String]
whatWentWrong ms = map logMessageToString (
  filter (sevGreaterThan 49) (
    filter isError (
      inOrder (build ms))))